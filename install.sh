...
theos="$(echo $REL | egrep -i '(cent|Scie|Red|Ubuntu|AlmaLinux|cloudlinux)' )"

if [ "$?" -ne "0" ]; then
	echo "Webuzo can be installed only on CentOS, Redhat, Ubuntu, AlmaLinux, Cloudlinux OR Scientific Linux"
	SERECHO "-1Webuzo can be installed only on CentOS, Redhat, Ubuntu, AlmaLinux, Cloudlinux OR Scientific Linux"
	echo "Exiting installer"
	exit 1;
fi
...
wget -N -O $EMPS/EMPS.tar.gz "http://files.softaculous.com/emps.php?arch=$ARCH&latest=1" >> $LOG 2>&1
