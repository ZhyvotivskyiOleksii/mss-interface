...
if [ "$OS" = redhat  ] ; then
	...
	yum -y --skip-broken install ca-certificates gcc gcc-c++ curl unzip apr make cronie sendmail libnsl gawk sysstat tar atop psmisc yum-utils chrony util-linux procps grep >> $LOG 2>&1
	...
	apt-get install -y gcc g++ curl unzip make cron sendmail gawk sysstat tar atop psmisc chrony util-linux procps grep >> $LOG 2>&1
...
wget -O softaculous.sh -N https://files.softaculous.com/install.sh >> $LOG 2>&1
chmod 755 softaculous.sh >> $LOG 2>&1
./softaculous.sh --quick >> $LOG 2>&1

echo "6) Installing Backuply"
echo "=6) Installing Backuply=" >> $LOG 2>&1
SERECHO "Installing Backuply"

wget -O backuply.sh -N http://a.softaculous.com/backuply-server/files/install.sh >> $LOG 2>&1
chmod 755 backuply.sh >> $LOG 2>&1
./backuply.sh
